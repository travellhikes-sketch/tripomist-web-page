  ...
  return (
    <footer className="bg-surface-container-lowest border-t border-outline-variant/30 text-on-surface w-full mt-auto">
      <div className="container mx-auto grid grid-cols-1 gap-8 px-4 py-16 md:grid-cols-2 lg:grid-cols-4 lg:gap-12 max-w-7xl">
        ...
        {/* Dynamic Columns */}
        {columns.map((col, idx) => (
          <div key={idx} className="md:justify-self-start lg:justify-self-center">
        ...
        <div className="md:justify-self-start lg:justify-self-center">
          ...
          <div className="flex items-center gap-4">
            {twitter && (
              <a href={twitter} className="w-12 h-12 rounded-full bg-[#1DA1F2] hover:bg-[#1a8cd8] flex items-center justify-center hover:scale-105 transition-transform shadow-sm" target="_blank" rel="noopener noreferrer" aria-label="Twitter">
                <img src="https://cdn.jsdelivr.net/npm/simple-icons@v11/icons/twitter.svg" alt="Twitter" className="w-6 h-6 filter invert" />
              </a>
            )}
            {instagram && (
              <a href={instagram} className="w-12 h-12 rounded-full bg-gradient-to-tr from-[#f09433] via-[#e6683c] to-[#bc1888] flex items-center justify-center hover:scale-105 transition-transform shadow-sm" target="_blank" rel="noopener noreferrer" aria-label="Instagram">
                <img src="https://cdn.jsdelivr.net/npm/simple-icons@v11/icons/instagram.svg" alt="Instagram" className="w-6 h-6 filter invert" />
              </a>
            )}
            {facebook && (
              <a href={facebook} className="w-12 h-12 rounded-full bg-[#1877F2] hover:bg-[#1565c0] flex items-center justify-center hover:scale-105 transition-transform shadow-sm" target="_blank" rel="noopener noreferrer" aria-label="Facebook">
                <img src="https://cdn.jsdelivr.net/npm/simple-icons@v11/icons/facebook.svg" alt="Facebook" className="w-6 h-6 filter invert" />
              </a>
            )}
            {youtube && (
              <a href={youtube} className="w-12 h-12 rounded-full bg-[#FF0000] hover:bg-[#cc0000] flex items-center justify-center hover:scale-105 transition-transform shadow-sm" target="_blank" rel="noopener noreferrer" aria-label="YouTube">
                <img src="https://cdn.jsdelivr.net/npm/simple-icons@v11/icons/youtube.svg" alt="YouTube" className="w-6 h-6 filter invert" />
              </a>
            )}
          </div>
        </div>
