  ...
  useEffect(() => {
    async function fetchPackages() {
      ...
      if (error) {
        ...
        let categoryPackages = activePackages.filter(pkg => pkg.category === 'Trek' || pkg.title.toLowerCase().includes('trek'));
      ...
      <section className="w-full max-w-6xl mx-auto px-4 pt-12 pb-6">
        <div className="flex-1">
          <h2 className="font-headline-md text-headline-md text-on-surface font-bold mb-4">
            About Trekking Departure
          </h2>
          <ReadMoreText text="Conquer the peaks and explore the unseen with our Trekking Departures. Whether you are a beginner or a seasoned trekker, we have the perfect trail waiting for you." />
        </div>
      </section>
