...
-- ============================================================================
-- TripoMist: Secure Checkout Database RPCs (Service Role Only)
-- File:    20260731_voucher_checkout_rpc.sql
-- Status:  REVIEW DRAFT — NOT YET APPROVED FOR EXECUTION
-- Author:  Schema Audit 2026-07-31
-- ============================================================================

-- ============================================================================
-- 1. reserve_coupon_for_checkout
-- ============================================================================
...
BEGIN
    ...
    IF v_booking.user_id IS NULL THEN
        RAISE EXCEPTION 'Booking user_id is required.';
    END IF;
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Voucher code is invalid.';
    END IF;
    ...
    -- Verify booking and coupon belong to same user
    IF v_voucher.user_id IS NULL OR v_booking.user_id IS DISTINCT FROM v_voucher.user_id THEN
        RAISE EXCEPTION 'Authorization mismatch: Booking and Coupon ownership must match.';
    END IF;

    -- Verify voucher active status and validity window
    IF v_voucher.status NOT IN ('active', 'partially_used') THEN
        RAISE EXCEPTION 'Coupon is not active.';
    END IF;

    IF v_voucher.valid_from > NOW() OR v_voucher.expires_at <= NOW() THEN
        RAISE EXCEPTION 'Coupon is outside its validity window.';
    END IF;
    ...
    IF FOUND THEN
        ...
        IF v_existing_reservation.expires_at <= NOW() THEN
            -- Expired-by-time but status is still pending/payment_pending -> requires reconciliation/cleanup
        ...
        ELSIF v_existing_reservation.status = 'payment_pending' THEN
            -- Still payment_pending and not expired -> payment might be in flight
    ...
    IF v_available_balance <= 0 THEN
        RAISE EXCEPTION 'Coupon balance is fully locked in other pending checkouts.';
    END IF;
    ...
    IF v_requested_discount <= 0 THEN
        RAISE EXCEPTION 'Calculated discount must be greater than zero.';
    END IF;
...
BEGIN
    ...
    -- Never attach reservation_id when booking has no voucher
...
BEGIN
    ...
    IF v_attempt.reservation_id IS NOT NULL THEN
        ...
        -- Verify reservation booking_id and user_id match the payment attempt
        IF v_reservation.booking_id IS DISTINCT FROM v_attempt.booking_id OR v_reservation.user_id IS DISTINCT FROM v_attempt.user_id THEN
...
BEGIN
    ...
    IF v_booking.user_id IS DISTINCT FROM v_attempt.user_id THEN
    ...
    IF v_attempt.reservation_id IS NOT NULL THEN
        ...
        END IF;
        IF v_reservation.user_id IS DISTINCT FROM v_booking.user_id OR v_reservation.user_id IS DISTINCT FROM v_attempt.user_id THEN
            RAISE EXCEPTION 'Reservation user ID mismatch.';
        END IF;
        IF v_reservation.voucher_id IS DISTINCT FROM v_booking.voucher_id THEN
        ...
        IF v_voucher.user_id IS DISTINCT FROM v_booking.user_id THEN
            RAISE EXCEPTION 'Voucher user ownership mismatch.';
        END IF;
        ...
        -- Deduct balance using v_balance_after
        ...
        -- Write immutable redemption audit ledger
...
-- ============================================================================
-- 5. finalize_full_coupon_checkout
-- ============================================================================
...
BEGIN
    ...
    -- Verify booking, reservation, voucher and user IDs match
    IF v_reservation.user_id IS DISTINCT FROM v_booking.user_id THEN
        RAISE EXCEPTION 'Reservation user ID mismatch.';
    END IF;
    IF v_reservation.voucher_id IS DISTINCT FROM v_booking.voucher_id THEN
    ...
    -- 5. Deduct balance using v_balance_after
    UPDATE public.vouchers SET
        remaining_amount = v_balance_after,
        status = CASE WHEN v_balance_after = 0 THEN 'redeemed' ELSE 'partially_used' END,
        updated_at = NOW()
...
-- ============================================================================
-- 6. release_checkout_reservation
-- ============================================================================
