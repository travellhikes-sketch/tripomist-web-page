-- Vercel/Voucher cancellation constraint fix migration
-- File: supabase/migrations_review/20260802_cancel_booking_constraint_fix.sql
...
BEGIN
    ...
    INSERT INTO public.booking_activity_logs (
        ...
        v_admin_id,
        'cancel',
        jsonb_build_object('booking_status', v_booking.booking_status, 'payment_status', v_booking.payment_status)::text,
        jsonb_build_object('booking_status', 'cancelled', 'payment_status', CASE WHEN p_refund_status = 'Fully Refunded' THEN 'refunded' ELSE v_booking.payment_status END)::text,
