...
) RETURNS TABLE (checkout_lead_id UUID, lead_number TEXT)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
AS $$
BEGIN
    INSERT INTO public.checkout_leads (lead_token_hash)
    VALUES (p_lead_token_hash)
    RETURNING id, lead_number INTO checkout_lead_id, lead_number;
END;
$$;
...
END;
$$;
REVOKE EXECUTE ON FUNCTION public.create_checkout_lead(TEXT) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_checkout_lead(TEXT) TO service_role;
